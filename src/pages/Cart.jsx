...
export const Cart = () => {
    ...
    const pizzas=Object.keys(items).map(el=>items[el][0])
        ...
        <div className="container container--cart">
            <div className="cart">
                ...
                <div className="content__items">
                    {pizzas.length && pizzas.map((item,index)=><CartItem key={`${item.name}_${index}`} name={item.name} type={item.selectType} size={item.selectSize} price={item.price} />)}
                   {/*<CartItem name={name} type={selectType} size={selectSize} price={price}/>*/}
